...
//                let profileImageURL = data?["profileImageURL"] as? String ?? ""
                ...
                DispatchQueue.main.async {
                    self.userProfile = User(userUid: userUid, profileUrl: profileUrl, statusMessage: statusMessage, userEmail: userEmail, userId: userId, userName: userName)
                }
            }
        }
    }
    ...
    func fetchUserPosts(userID: String) {
        db.collection("users").document(userID).collection("Post").getDocuments { querySnapshot, error in
...
//                    let date = data["date"] as? Date ??
                    ...
                    return Post(writerUid: writerUid, date: date, imageURL: imageURL, postId: postId, text: text, writerName: writerName, writerProfileUrl: writerProfileUrl)
                }
